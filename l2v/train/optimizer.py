...
class DelayedWarmupSchedule:
  ...
  def __init__(
    ...
    for group in optimizer.param_groups:
      ...
      group["step"] = 0
      if group.get("delay"):
        # Start at zero, freeze gradients for efficiency
        group["lr"] = 0
...
@ParameterSet.register("adapter")
class Adapter(ParameterSet):
  def get(self, model) -> List[nn.Parameter]:
    return list(model.adapter.parameters())


@ParameterSet.register("non-adapter")
class NoNAdapter(ParameterSet):
  def get(self, model) -> List[nn.Parameter]:
    adapt = set(model.adapter.parameters())
    return list(x for x in model.parameters() if x not in adapt)
...
class AdamWBuilder(OptimizerBuilder):
  ...
  def build(self, model: torch.nn.Module, epoch_size, n_epochs):
    if self.parameter_groups is None:
      ...
      parameters = get_param_groups(model, self.parameter_groups if self.parameter_groups else [])
